...
Figure dimensions: width, height (Max: 65 x 65. Default dependent on number of slices and axes)
...
class STATSHeatmapMenu(QtWidgets.QWidget):
    ...
    # create GUI
        ...
        mandatory_lbl=QtWidgets.QLineEdit("MANDATORY Arguments")
        mandatory_lbl.setFrame(False)
        mandatory_lbl.setStyleSheet("QLineEdit {background-color: rgb(240,240,240)}")
        mandatory_lbl.setReadOnly(True)
        optional_lbl=QtWidgets.QLineEdit("OPTIONAL Arguments")      
        optional_lbl.setFrame(False)
        optional_lbl.setStyleSheet("QLineEdit {background-color: rgb(240,240,240)}")
        optional_lbl.setReadOnly(True)  
        ...
        self.layout = QtWidgets.QFormLayout()
        self.layout.addRow(mandatory_lbl)
        self.layout.addRow(self.lbl1, self.btn1)
        self.layout.addRow("Voxel Size/Resolution - Choose from 10, 25, or 50 um", self.voxels)
        self.layout.addRow(QtWidgets.QLabel("______________________________________________________________________________________________________"))
        self.layout.addRow(optional_lbl)
        self.layout.addRow(self.lbl2, self.btn2)         
        self.layout.addRow(group2_note)
        self.layout.addRow("Gaussian smoothing sigma (Non-negative number. ex: 1, 2 or 2.5)", self.gauss)
        self.layout.addRow("percentile (%) threshold for registration-to-input data check svg animation", self.percentile)
        ...
        self.layout.addRow("sagittal slicing: start_slice_number, interval, number_of_slices, number_of_rows, number_of_columns", self.slice_layout[0])
        self.layout.addRow("Enable Custom Coronal Slicing", self.coronal_enable)
        self.layout.addRow("coronal slicing: start_slice_number, interval, number_of_slices, number_of_rows, number_of_columns", self.slice_layout[1])
        self.layout.addRow("Enable Custom Axial Slicing", self.axial_enable)
        self.layout.addRow("axial slicing: start_slice_number, interval, number_of_slices, number_of_rows, number_of_columns", self.slice_layout[2])        
        self.layout.addRow(custom_slice)
        self.layout.addRow("Enable Custom Figure Dimensions", self.fig_enable)
        self.layout.addRow(default_dim)
        self.layout.addRow("Figure Width and Height (inches)", dim_layout)       
        self.layout.addRow(self.lbl3, self.btn3)
        self.layout.addRow(output_note)
        self.layout.addRow("Output/Figure Filename Group 1. Default: group_1", self.outfiles_g1)
        self.layout.addRow(group2_selected)
        self.layout.addRow("Output/Figure Filename Group 2", self.outfiles_g2)
        self.layout.addRow("Output/Figure Filename Difference of Groups (Group 2 - Group 1)", self.outfiles_dif)
        self.layout.addRow("Figure Extension", self.extensions)
        self.layout.addRow("Dots Per Inch", self.dots_per_inch)
        self.layout.addRow(help_run_layout)
        self.setLayout(self.layout)
        ...
        btn4.clicked.connect(lambda: self.msgbox('Matplotlib Colourmap Options: \n' +  str(plt.colormaps()[:]).strip('[]').replace("'", "")))
    ...
    def print_input(self):
        ...
        def cmap_check(cmap, select_box):
            if cmap == "":
                ...
                return(True)
        def contain_check(arr, value, select_box):
            if value not in arr:
                select_box.setStyleSheet("background-color :rgb(255,255,159)")
                return(self.msgbox("Must Select Valid Voxel Size"))
            else:
                return(True)
        ...
        self.closing=min(contain_check(["10", "25", "50"], voxel, self.voxels), self.closing)
        if self.closing:
            voxel=int(voxel)
        ...
        if self.fig_status:
            ...
            for i, axis in zip(cuts_ind, cuts[cuts_ind]):
                max_col = min(axis[4], 65)
                if max_col * dots_per_inch > 65535:
                    self.axis[i][4].setStyleSheet("background-color:rgb(255,255,159)")
                    self.closing = self.msgbox("Width Resolution exceeds maximum. Cols: {}. DPI: {}. (Min value of either Max Width or Max Col) x DPI = {} x {} = {}. Cannot exceed 65535. Note: Max Width is capped at 65".format(
                                    axis[4], dots_per_inch, max_col, dots_per_inch, max_col * dots_per_inch))
            #validate row
            max_row=min(sum([i[3] for i in cuts[cuts_ind]]), 65)
            if max_row * dots_per_inch > 65535:
                [self.axis[i][3].setStyleSheet("background-color:rgb(255,255,159)") for i in cuts_ind]
                self.closing = self.msgbox("Height Resolution exceeds maximum. Rows: {}. DPI: {}. (Min value of either Max Height or Total Rows) x DPI = {} x {} = {}. Cannot exceed 65535. Note Max Height is capped at 65".format(
