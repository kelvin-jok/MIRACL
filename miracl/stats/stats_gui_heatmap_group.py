...
class STATSHeatmapMenu(QtWidgets.QWidget):
    ...
    # create GUI
        ...
        #self.scrollArea.setWidget(self.optional_box)
        ...
        opt_form.addRow("Output/Figure Filename Group 1. Default: group_1", self.outfiles_g1)
        ...
        self.scrollArea.setWidgetResizable(True)
        layout.addWidget(self.scrollArea)
        #layout.addWidget(self.optional_box)
        layout.addLayout(help_run_layout)
        self.setLayout(layout)

        #self.scrollArea.resize(1175, 725)
        self.scrollArea.resize(0,0)
    ...
    def optional_arg(self, box, check, status):
        ...
        check=getattr(self,check).isChecked()
        if check:
            minsize = box.minimumSizeHint()
            box.resize(1175, 725)
            self.resize(1200, 500)
            #minsize.setHeight(box.minimumSizeHint().height())
            #minsize.setWidth(box.minimumSizeHint().width())
        else:
            minsize = box.minimumSizeHint()
            box.resize(0, 0)
            #minsize = box.minimumSizeHint()
