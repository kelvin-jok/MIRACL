...
def smooth_plot(temp, img, outdir, outfile, x, y, z, cut_coords, sigma, fig, axes, cmap,
    ...
    if x != -1:
        for i in range(slice_len[x]):
            # binarize mean_img with annotation slice
            mean_img = img[:, :, cut_coords[x][i]] * binarize(mask_slices[x][i])
            #threshold image
            ...
            #brain template
            mean_img = np.where((mean_img == 0) & (temp[x][i] > 0), img_max / (255) * 1.5, mean_img)
            # edge/contour
    ...
    if y != -1:
        for i in range(slice_len[y]):
            # binarize mean_img with annotation slice
            mean_img = img[cut_coords[y][i], :, :] * binarize(mask_slices[y][i])
            #threshold image
            if np.amin(mean_img) < 0:
                mean_img[np.where((mean_img < img_max *0.02) & (mean_img > -img_max *0.02))] = 0
            else:
                mean_img[np.where(mean_img < img_max *0.02)] = 0
            # brain template
            mean_img = np.where((mean_img == 0) & (temp[y][i] > 0), img_max / (255) * 1.5, mean_img)
            # edge/contour
            mask_img = (
    ...
    if z != -1:
        for i in range(slice_len[z]):
            # binarize mean_img with annotation slice
            mean_img = img[:, cut_coords[z][i], :] * binarize(mask_slices[z][i])
            #threshold image
            if np.amin(mean_img) < 0:
                mean_img[np.where((mean_img < img_max * 0.02) & (mean_img > -img_max * 0.02))] = 0
            else:
                mean_img[np.where(mean_img < img_max * 0.02)] = 0
            #brain template
            mean_img = np.where((mean_img == 0) & (temp[z][i] > 0), img_max / (255) * 1.5, mean_img)
            # edge/contour
        ...
        for axis, tag in zip([sagittal, coronal, axial], ["s/sagittal", "c/coronal", "a/axial"]):
            if type(axis) != float:
                ...
                if max(cut_coords[-1]) > img_shape[ind]:
                    raise Exception(
                        "AXIS {} slice {}, is greater than max slice size {}".format(tag, max(cut_coords[-1]),
                                                                                     img_shape[ind]))
...
def main(args):
    ...
    print("Step 2/{} : Completed Group 1 Mean and QC Reg Check SVG File".format(4 + int(multi) * 3))
    ...
    print("Step 3/{} : Completed Extraction of Atlas Slices".format(4 + int(multi) * 3))
