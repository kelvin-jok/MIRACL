...
def helpmsg():
    ...
    example: miracl stats heatmap_group -g1 group_1_dir -v 25 
        ...
        f. figure dimensions: width, height (Max: 65 65. Default dependent on number of slices and axes)
...
def parse_inputs(parser, args):
    ...
    for axis, tag in zip( [sagittal, coronal, axial], ["s/sagittal", "c/coronal", "a/axial"]): 
        if type(axis) != float:
            ...
            min_check(axis[3]*axis[4], axis[2], "".join(("-", tag +" number_of_rows x number_of_columns is less than number_of_slices. row x col value: {} x {} = {}. Number_of_slices {}".format(axis[3], axis[4], axis[3]*axis[4], axis[2]))))
            row.append(axis[3])
            col.append(axis[4])
    ...
    #Validate DPI x row/col or height/width doesn't exceed maximum pixel size. Note: max width and height capped at 65
    if figure_dim==None and len(row)>0:
        max_col=min(np.amax(col),65)
        max_row=min(sum(row),65)
        min_check(65536/dpi, max_row, "".join(("Axis Rows and dpi, Height Resolution exceeds maximum. Rows: {}. DPI: {}. (Largest value of either Max Height or Total Rows) x DPI = {} x {} = {}. Cannot exceed 65535. Note: Max Height is capped at 65".format(row, dpi, max_row, dpi, max_row*dpi))))
        min_check(65536/dpi, max_col, "".join(("Axis Columns and dpi, Width Resolution exceeds maximum. Cols: {}. DPI: {}. (Largest value of either Max Width or Max Col) x DPI = {} x {} = {}. Cannot exceed 65535. Note: Max Width is capped at 65".format(col, dpi, max_col, dpi, max_col*dpi))))
    if figure_dim!=None:    
        min_check(65536/dpi, figure_dim[0], "".join(("-f/figure_dim and -dpi, Height resolution exceeds maximum. Height: {}. DPI: {}. Height x DPI = {}. Cannot exceed 65535".format(figure_dim[0], dpi, figure_dim[0]*dpi))))
        min_check(65536/dpi, figure_dim[1], "".join(("-f/figure_dim and -dpi, Width resolution exceeds maximum. Width: {}. DPI: {}. Width x DPI = {}. Cannot exceed 65535".format(figure_dim[1], dpi, figure_dim[1]*dpi))))

    multi=False
    #validate outfile names
    if isinstance(g2, str):
        find_value(3, [len(outfile)], False,
            "-o Must enter three arguments. Detected {} Arguments: {} \n Names must use underscore instead of space. \n Ex. ACCEPTABLE -o control_group treated_group difference_group \n UNACCEPTABLE -o control group treated group difference group".format(len(outfile), outfile))
        multi=True
    else:
        if outfile !=['group_1', 'group_2', 'group_difference']:
            find_value(1, [len(outfile)], False,
                "-o Must enter one argument. Detected {} Arguments: {} \n Names must use underscore instead of space. \n Ex. ACCEPTABLE -o control_group \n UNACCEPTABLE -o control group ".format(len(outfile), outfile))
...
def grp_mean(input_path, brain_template, outdir,  x, y, z, percentile):
    ...
    for root, dirnames, filenames in os.walk(input_path):
        for filename in fnmatch.filter(filenames, '*.nii.gz'):
            ...
            max_val=max(max_val, np.amax(smooth_img[:,:,list(range(nib_file.shape[2]//2-45,nib_file.shape[2]//2+15*3+1,15))]))  
            max_val=max(max_val, np.amax(smooth_img[list(range(nib_file.shape[0]//2-45,nib_file.shape[0]//2+15*3+1,15)),:,:]))
            max_val=max(max_val, np.amax(smooth_img[:,list(range(nib_file.shape[1]//2-45,nib_file.shape[1]//2+15*3+1,15)), :]))
    ...
    if sample == 0:
        raise Exception('{} is empty or does not contain .nii.gz files ... please check path/file contents and rerun script'.format(input_path))
...
#-----smooth mean data and superimpose onto plot-----
def meanplot(img, outdir, outfile, x, y, z, cut_coords, sigma, fig, axes, cmap,
             group, mask, mask_slices, slice_len):

    img = gaussian_filter(img, sigma=(sigma, sigma, sigma))
    mean_nii_export(img, outdir, outfile, mask)
    m_norm = cmap_norm(img, x, y, z, cut_coords, group)

    if x != -1:
        for i in range(slice_len[x]):
            c = axes[x][i].imshow(np.rot90(img[:, :, cut_coords[x][i]] * binarize(mask_slices[x][i]), 3), norm=m_norm, cmap=cmap, zorder=2, interpolation='none')
    ...
    if y != -1:
        ...
        for i in range(slice_len[y]):
            c = axes[y][i].imshow((img[cut_coords[y][i], :, :] * binarize(mask_slices[y][i])),norm=m_norm,cmap=cmap,zorder=2, interpolation='none')
    ...
    if z != -1:
        ...
        for i in range(slice_len[z]):
            c = axes[z][i].imshow((img[:, cut_coords[z][i], :] * binarize(mask_slices[z][i])),norm=m_norm, cmap=cmap, zorder=2, interpolation='none')
...
#-----superimpose brain template onto plot -----
def tmp_plot(temp, cmap, x, y, z, axes, slice_len):
    if x != -1:
        for i in range(slice_len[x]):
            axes[x][i].imshow(np.rot90(temp[x][i], 3),cmap=cmap,vmin=0.001, zorder=1, interpolation='none')

    if y != -1:
        for i in range(slice_len[y]):
            axes[y][i].imshow((temp[y][i]),cmap=cmap,vmin=0.001,zorder=1, interpolation='none')

    if z != -1:
        for i in range(slice_len[z]):
            axes[z][i].imshow((temp[z][i]),cmap=cmap,vmin=0.001, zorder=1, interpolation='none')

#-----superimpose outline onto plot | annotation file -> edges -----
def roberts_edge_plot(img, cmap, x, y, z, axes,slice_len):

    if x != -1:
        for i in range(slice_len[x]):
            temp_img = ((255 * (img[x][i] - np.min(img[x][i])) / np.ptp(img[x][i])).astype(float32))
            roberts_edge = (filters.apply_hysteresis_threshold(filters.roberts(temp_img), 0, 1).astype(int))
            axes[x][i].imshow(np.rot90(roberts_edge, 3), cmap=cmap, zorder=3, interpolation='none')

    if y != -1:
        for i in range(slice_len[y]):
            temp_img = ((255 * (img[y][i] - np.min(img[y][i])) / np.ptp(img[y][i])).astype(float32))
            roberts_edge = (filters.apply_hysteresis_threshold(filters.roberts(temp_img), 0, 1).astype(int))
            axes[y][i].imshow((roberts_edge), cmap=cmap, zorder=3, interpolation='none')

    if z != -1:
        for i in range(slice_len[z]):
            temp_img = ((255 * (img[z][i] - np.min(img[z][i])) /np.ptp(img[z][i])).astype(float32))
            roberts_edge = (filters.apply_hysteresis_threshold(filters.roberts(temp_img), 0, 1).astype(int))
            axes[z][i].imshow((roberts_edge), cmap=cmap, zorder=3, interpolation='none')

#----- check if slice is blank image then extract slice -----
    ...
    if x != -1:
        ...
        for i in (cut_coords[x]):
            if np.min(img[:, :, i]) != np.max(img[:, :, i]):
                ...
                raise Exception("{} AXIS s/sagittal slice {} is blank".format(atlas, i))
    ...
    if y != -1:
        ...
        for i in (cut_coords[y]):
            if np.min(img[i, :, :]) != np.max(img[i, :, :]):
                ...
                raise Exception("{} AXIS c/coronal slice {} is blank".format(atlas, i))
    ...
    if z != -1:
        ...
        for i in (cut_coords[z]):
            if np.min(img[:, i, :]) != np.max(img[:, i, :]):
                ...
                raise Exception("{} AXIS a/axial slice {} is blank".format(atlas, i))
        slices.append(z_slices)
    return(slices)

#----- slicing parameters -----
def slice_display(img_shape, sagittal, coronal, axial, x, y, z):
    ...
    if type(sagittal) == float and type(coronal) == float and type(axial) == float:
        for i in range(3):
            cut_coords.append(list(range(ceil(img_shape[i] / 17 * 3), ceil(img_shape[i] / 17) * 15 + 1, ceil(img_shape[i] / 17 * 2))))
        cut_len=3    
    #custom slices
    else:
        ind = 0
...
#-----colour map boundaries-----  
def cmap_norm(img, x, y, z, cut_coords, group):
    #find absolute maximum of selected slices
    img_max=[]
    if x != -1:
    ...
    if group == 2:
        ...
        img_min = 0
    m_norm = mcolors.Normalize(vmin=img_min, vmax=img_max, clip=False)
    return (m_norm)

#-------heatmap colors -------
def colormap(c1, c2, group, layer):

    #template colour
    if layer == 1:
        cmap = mcolors.ListedColormap(c2)
    ...
    #mean data colourmap
        ...
        if group != 2:
            ...
            #threshold out first two shades of colourmap by making white (transparent)
            colors1[0:2, :] = white
            ...
            #threshold out two middle shades of positive and negative bins around zero for colourmap by making white (transparent)
            colors[253:258, :] = white
            cmap = mcolors.LinearSegmentedColormap.from_list('my_colormap', colors)
    #edge/contour colour
    elif layer == 3:
        cmap = mcolors.ListedColormap([c1, c2])
...
def figure_setup(cut_coords, cut_len, sagittal, coronal, axial, figure_dim, dpi):
    ...
    #height ratio for subfigures
    for x in [sagittal, coronal, axial]:
        if type(x)!=float:
            rows.append(x[3])
            cols.append(x[4])
            custom=True
    ...
    #figure dimensions. Cap at 65 in x 65 in
...
def plot(mean_img, mask, group, vox, cut_coords, cut_len, sagittal, coronal, axial, figure_dim, outdir,
         outfile, sigma, cn, cp, mask_slices, temp_slices, x, y, z, extension,
         dpi):

    fig, axes, slice_len = figure_setup(cut_coords, cut_len, sagittal, coronal, axial, figure_dim, dpi)

    #call heatmap plot layers
    tmp_plot(temp_slices, colormap('none', 'dimgray', group=group, layer=1), x, y, z, axes, slice_len)
    roberts_edge_plot(mask_slices,colormap('none', '#fffaed', group=group, layer=3), x, y, z, axes, slice_len)
    meanplot(mean_img, outdir, outfile, x, y, z, cut_coords, sigma, fig, axes, colormap(cn, cp, group=group, layer=2), group, mask, mask_slices, slice_len)
    ...
    if extension == "tiff":
        fig.savefig("".join((outdir, "/", outfile, "_mean_plot.tiff")),bbox_inches="tight",pil_kwargs={"compression": "tiff_lzw"}, pad_inches=0)
    else:
...
def main(args):
    ...
    parser = parsefn()
    g1, g2, vox, sigma, percentile, cp, cn, sagittal, coronal, axial, x, y, z, figure_dim, outdir, outfile, extension, dpi, multi = parse_inputs(parser, args)
    
    print("Step 1/{}: Completed Input Argument Validation".format(4+int(multi*2)))
    ...
    print("Step 2/{} : Completed Group 1 Mean and QC Reg Check SVG File".format(4+int(multi)*2))

    cut_len, cut_coords = slice_display(img_shape, sagittal, coronal, axial, x, y, z)
    ...
    print("Step 3/{} : Completed Extraction of Atlas Slices".format(4+int(multi)*2))

    #plot first heatmap
    plot(img1, mask, 0, vox, cut_coords, cut_len,  sagittal, coronal, axial, figure_dim, outdir, outfile[0], sigma, cn, cp, mask_slices, temp_slices, 
         x, y, z, extension, dpi)
    print("Step 4/{} : Completed {} Heatmap Figure and Mean Nii File".format(4+int(multi*2), outfile[0]))
    ...
    if multi == True:
        ...
        plot(img2, mask, 1, vox, cut_coords, cut_len,  sagittal, coronal, axial,figure_dim, outdir, outfile[1], sigma, cn, cp, mask_slices,
             temp_slices, x, y, z, extension, dpi)
        print("Step 5/6 : Completed {} Heatmap Figure and Mean Nii File".format(outfile[1]))         
        plot(img2 - img1, mask, 2, vox, cut_coords, cut_len, sagittal,coronal, axial,figure_dim, outdir, outfile[2], sigma, cn, cp, mask_slices,
             temp_slices, x, y, z, extension, dpi)
        print("Step 6/6 : Completed {} Heatmap Figure and Mean Nii File".format(outfile[2])) 
    end = time.time()
    print("Completed in {} seconds".format(end - start_time))
